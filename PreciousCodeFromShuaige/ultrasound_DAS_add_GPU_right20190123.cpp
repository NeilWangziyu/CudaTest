...
__global__ void theta_calculate_WithCuda(cuDoubleComplex *d_init_M1, cuDoubleComplex *beamDASDAS)
{
	...
	if (idx < 501 * 121)
	{
		...
		cuDoubleComplex x_pointer_new = make_cuDoubleComplex(0, 0);
		for (Trans_elements = emit_start; Trans_elements <= (emit_start + emit_pitch * (emit_aperture - 1)); Trans_elements++)
		{
			...
			for (i = theta_start; i <= theta_end; i++)
			{
				...
				x_pointer_new = cuCadd(x_pointer_new, d_init_M1[temp_2]);
		...
		//	}
		//}
		beamDASDAS[(b - 1) * 121 + (a - 1)] = make_cuDoubleComplex(cuCreal(x_pointer_new) / ((double)(M_elements*M_elements)), cuCimag(x_pointer_new) / ((double)(M_elements*M_elements)));
...
int main()
{
	...
	//init_imaginary = (double *)mxGetImagData(pA);
	//init_imaginary = (double *)mxGetPi(pA);
	init_real = mxGetPr(pA);
	init_imaginary = init_real + 1;
	...
	dim3 threadsPerBlock(1, 501);
	dim3 numBlocks(1, 121);
	...
	theta_calculate_WithCuda << <numBlocks, threadsPerBlock >> > (d_init_M1, beamDASDAS);
